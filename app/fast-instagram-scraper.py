...
def ilink(cursor=""):
    if location_or_hashtag == "location":
        instalink = 'https://instagram.com/graphql/query/?query_hash=' + location_hash + '&variables={"id":"' + str(
...
def add_locations_data_to_cleaned_node(nodelist, just_clean=False):
    if just_clean:
        ...
        nodelist = [add_location_data(delete_keys(i["node"])) for i in
                    nodelist]  # chained functions and list comprehension
        return nodelist


# "berlin,999111555,[1234567,hamburg],[munich,cologne]" ->
# ['berlin', '999111555', '[1234567,hamburg]', '[munich,cologne]']
...
def download_img(session, i_img_name, i_img_link):
    try:
        ...
        with open('{}{}.png'.format(out_dir, i_img_name),
                  'wb') as out_file:  # saves png file always, if status is 200 file will have 1kb only
    ...
    with TorRequests() as tor_requests:
        with tor_requests.get_session() as sess, tqdm(total=0) as pbar:
            ...
            while i < max_requests:  # enter main loop
                ...
                if idata["data"][location_or_hashtag] is None:
                ...
                # json saving logic 
                    ...
                    if run_number != "":
                        run_number_loop = "_" + str(float(datetime.datetime.now().timestamp())).replace(".", "") + \
                                          "_" + run_number
                        # change to some index number or just leave the timestamp but watch out for duplicates!
                    else:
                        run_number_loop = "_" + str(float(datetime.datetime.now().timestamp())).replace(".",
                                                                                                        "")
                        # change to some index number or just leave the timestamp but watch out for duplicates!
                ...
                this_cursor = edge_to_media["page_info"][
                    "end_cursor"]  # this_cursor is next page cursor means unscraped page
                ...
                if not edge_to_media["page_info"]["has_next_page"] and this_cursor is None:
                    print("Successfully scraped until last page for {}".format(object_id_or_string))
                    open("{}{}_last_cursor.txt".format(out_dir, object_id_or_string), 'a').write(
                        "Last_Cursor_empty" + "\n")
                    return "no_more_page"
...
if __name__ == "__main__":
    ...
    object_id_or_string = args.object_id_or_string  # "1034863903"
    # a string for hashtag an int for location id i.e. 12345678 or list
    ...
    wait_between_requests = args.wait_between_requests
    # time in seconds to wait for next requests adding up to normal execution time ~ 4-8 seconds
    ...
    if not args.list:
        ...
        if args.last_cursor:
            ...
            last_cursor = Path("{}{}_last_cursor.txt".format(out_dir, object_id_or_string)).read_text().split("\n")[
                -2]  # reads last cursor
            if run_number != "":
                run_number = "_" + str(float(datetime.datetime.now().timestamp())).replace(".","") +\
                             "_" + run_number  # change to some index number or just leave the timestamp
                # but watch out for duplicates!
            else:
                run_number = "_" + str(float(datetime.datetime.now().timestamp())).replace(".", "")
                # change to some index number or just leave the timestamp but watch out for duplicates!
            scrape()
    ...
    # Scrape multiple hashtags and/or location ids (list)
        ...
        for index, element in enumerate(tqdm(scrape_items_list)):
            ...
            # use this try except block when scraping over a longer period so that any ocurring error doesn't
            # stop the main loop
            # try:
            #     scrape()
            # except:
